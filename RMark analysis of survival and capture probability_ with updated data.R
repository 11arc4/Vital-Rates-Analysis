...
ggplot(FemaleResults %>% filter(seSYReturn<0.2 & seASYReturn<0.2), aes(x=Year))+
...
write.csv(FemaleResults, file="Model Averaged Female Yearly Apparent Survival_updated.csv", row.names = F, na="")
  
png(filename = "~/Masters Thesis Project/Committee Meetings/Dec 15 2017/Apparent Survival with Discrete year.png",
    width =600, height = 300)

FemaleResults_notupdated <- read.csv("Model Averaged Female Yearly Apparent Survival.csv")


plot(FemaleResults_notupdated$SYReturn~FemaleResults$SYReturn)
plot(FemaleResults_notupdated$ASYReturn~FemaleResults$ASYReturn)
#OK so I really do need to change this for my vital rates analysis. Oops. Good thing I caught this issue!!!
